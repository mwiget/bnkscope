...
Only the exporter. `bnkscope-api`, `bnkscope-frontend` and `bnkscope-mcp` are
built from source by `./bnkscope up` on whichever machine runs them, and are
never pushed to a registry.

Publishing them was tried and removed (D-041). It saved a build and nothing
else: `docker-compose.yml`, `telemetry/prometheus/prometheus.yml`,
`telemetry/grafana/**` and `VERSION` are all bind-mounted from the checkout, so
a released image still needs the repository beside it — and running from a
second clone silently re-points a live Prometheus and Grafana at that clone's
files.

`bnkscope-tmm-stat-exporter` is the exception because it is the exception in
kind: it is injected into `f5-tmm` pods on the operator's clusters, which have
no checkout and must pull it from a registry. That is the one image worth
building multi-arch, signing, and attaching an SBOM to.
...
Normally none of this is run by hand — the Release workflow does it. These are
the same steps, for when it has to be done locally.

```bash
# 1. Build + push the exporter first
...
The script signs by digest (not tag) and attaches two attestations:
...
```bash
IMAGE=ghcr.io/mwiget/bnkscope-tmm-stat-exporter:0.1.2
...
A tag works as well as a digest here; pin to `@sha256:…` when you want the check
to name one exact image.

Releases up to and including v0.1.2 also published `bnkscope-api`,
`bnkscope-frontend` and `bnkscope-mcp`, signed the same way. Those are no longer
produced — see **What is published, and what is not** above.
