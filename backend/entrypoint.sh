...
# Docker volumes are created as root but the container runs as bnkforge (uid
...
for dir in /app/data /app/keys; do
    if [ -d "$dir" ] && [ ! -w "$dir" ]; then
        echo "Warning: $dir is not writable by the bnkforge user"
        echo "   Run: docker exec -u root bnkscope-backend chown -R bnkforge:bnkforge $dir"
    fi
done
