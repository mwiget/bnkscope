...
class TestClusterCreateValidation:
    """POST /api/k8s/clusters — kubeconfig portability gate."""

    def test_path_ca_returns_422(self, client, admin_headers, sample_user):
    ...
    def test_path_client_cert_returns_422(self, client, admin_headers, sample_user):
        response = client.post(
            "/api/k8s/clusters",
            json={
                "name": "cert-cluster",
    ...
    def test_path_client_key_returns_422(self, client, admin_headers, sample_user):
        response = client.post(
            "/api/k8s/clusters",
            json={
                "name": "key-cluster",
    ...
    def test_path_token_file_returns_422(self, client, admin_headers, sample_user):
        response = client.post(
            "/api/k8s/clusters",
            json={
                "name": "token-cluster",
    ...
    def test_exec_unknown_returns_422(self, client, admin_headers, sample_user):
        response = client.post(
            "/api/k8s/clusters",
            json={
                "name": "oidc-cluster",
    ...
    def test_exec_aks_kubelogin_returns_422(self, client, admin_headers, sample_user):
        """kubelogin left the allowlist in Phase 5 — no binary, no Python equivalent."""
        response = client.post(
            "/api/k8s/clusters",
            json={
                "name": "aks-cluster",
        ...
        body = response.json()
        assert body["error"]["code"] == "KUBECONFIG_UNPORTABLE"
        assert "kubelogin" in body["error"]["details"]["user_message"]

    def test_multi_cluster_partial_returns_422(self, client, admin_headers, sample_user):
    ...
    def test_flat_eks_succeeds(self, client, admin_headers, sample_user, db):
    ...
    def test_mixed_data_and_path_succeeds(self, client, admin_headers, sample_user, db):
    ...
    def test_error_envelope_shape(self, client, admin_headers, sample_user):
...
class TestClusterUpdateValidation:
    ...
    def _make_cluster(self, client, admin_headers):
    ...
    def test_update_with_path_ca_returns_422(
        ...
        cluster_id = self._make_cluster(client, admin_headers)
        response = client.put(
